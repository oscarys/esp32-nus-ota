...
class OTAManager:
    ...
    def _on_start(self, fields):
        if self._state != self.IDLE:
            self._reply("OTA:REJECT:BUSY"); return
        if len(fields) < 3:
            self._reply("OTA:REJECT:BAD_ARGS"); return
        filename   = fields[0]
        size       = int(fields[1])
        ...
        if self.AUTH_TOKEN and token != self.AUTH_TOKEN:
            self._reply("OTA:REJECT:AUTH"); return
        if filename not in self.ALLOWED_FILES:
            self._reply("OTA:REJECT:FORBIDDEN"); return
        st = os.statvfs('/')
        if size > st[0] * st[3] * 0.8:
            self._reply("OTA:REJECT:NOSPACE"); return
        if version and self._is_downgrade(version):
            self._reply("OTA:REJECT:VERSION"); return
    ...
    def _on_data(self, seq_str, payload):
        if self._state not in (self.ARMED, self.RECEIVING):
            self._reply("OTA:ERR:NOT_READY"); return
        try:
            seq = int(seq_str)
        except ValueError:
            self._reply("OTA:ERR:BAD_SEQ"); return
        if seq == self._seq_expect - 1:
            self._reply("OTA:ACK:{}".format(seq)); return
        if seq != self._seq_expect:
            self._reply("OTA:ERR:{}".format(seq)); return
        try:
        ...
        try:
            ...
            self._reply("OTA:ERR:WRITE:{}".format(e)); return
    ...
    def _on_commit(self):
        if self._state != self.RECEIVING:
            self._reply("OTA:ERR:NOT_RECEIVING"); return
        ...
        if self._received != self._size:
            ...
            self._cleanup_tmp(); self._state = self.IDLE; return
        ...
        try:
            ...
            try: os.rename(target + '.bak', target)
            except OSError: pass
            self._reply("OTA:ERR:RENAME:{}".format(e))
            self._state = self.IDLE; return
        self._reply("OTA:OK")
        self._reset()
        import machine
        machine.reset()
    ...
    def _reset(self):
        self._state = self.IDLE
        self._filename = self._fh = None
        self._size = self._crc32_exp = self._crc32_run = 0
        self._chunk_size = 180
        self._seq_expect = self._received = 0
        self._tmp = '_ota_tmp'

    def _is_downgrade(self, incoming_version):
        try:
            from config import VERSION as current
            to_t = lambda v: tuple(int(x) for x in v.split('.'))
            return to_t(incoming_version) < to_t(current)
        except Exception:
            return False
